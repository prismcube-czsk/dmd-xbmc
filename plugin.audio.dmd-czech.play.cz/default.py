...
def SEZNAM():
	...
	link=response.read()
	response.close()
	match = re.compile('<title>.+?CDATA\[(.+?)\]\]></title>\s<description>.+?CDATA\[(.+?)\]\]></description>\s<shortcut>.+?CDATA\[(.+?)\]\]></shortcut>').findall(httpdata)
	for title,description,shortcut in match:
		addDir(title,shortcut,1,"http://api.play.cz/static/radio_logo/t200/"+shortcut+".png",description)
		...
		for suffix, bitrates in match2:
			...
			if suffix != "wma":
				for bitrate in match3:
					addLink(suffix+" "+bitrate+" kbps","http://api.play.cz/plain/getStream/"+shortcut+"/"+suffix+"/"+bitrate,"http://api.play.cz/static/radio_logo/t200/"+shortcut+".png","")
			else:
				for bitrate in match3:
					url = "http://api.play.cz/xml/getStream/"+shortcut+"/"+suffix+"/"+bitrate
					req = urllib2.Request(url)
					req.add_header('User-Agent', ' Mozilla/5.0 (Windows; U; Windows NT 5.1; en-GB; rv:1.9.0.3) Gecko/2008092417 Firefox/3.0.3')
					response = urllib2.urlopen(req)
					httpdata = response.read()
					...
					httpdata = httpdata.replace("\n","")
					httpdata = httpdata.replace("\t","")
					match = re.compile('<pubpoint>.+?CDATA\[(.+?)\]\]></pubpoint>').findall(httpdata)
					for stream in match:
						addLink(suffix+" "+bitrate+" kbps",stream,"http://api.play.cz/static/radio_logo/t200/"+shortcut+".png","")
