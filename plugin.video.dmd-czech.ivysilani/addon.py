...
try:
    ...
    well_known_error_messages = ['Programme not found!', 
                                 'Playlisturl is empty!']
    ###############################################################################
    ...
    def addDirectoryItem(label, url, ID=None, related=False, plot=None, title=None, date=None, duration=None, icon=_icon_, image=None, fanart=None, isFolder=True):
    ...
    def listProgrammelist(programmelist):
        ...
        for item in pList:
            ...
            if active:
                addDirectoryItem(title, url, ID=item.ID, related=True, plot=plot, date=date, image=item.imageURL)
    ...
    def listContext(what, ID, page):
        ...
        for item in l:
            ...
            if hasattr(item, "synopsis") and item.synopsis:
                plot = item.synopsis
            addDirectoryItem(item.title, _baseurl_ + "?play=" + item.ID, ID=item.ID, related=True, plot=plot, date=date, image=item.imageURL)
    ...
    if play:
        ...
        try:
            ...
            if ex.message in well_known_error_messages:
                xbmcgui.Dialog().notification(_scriptname_, ex.message, xbmcgui.NOTIFICATION_ERROR)    
            else:
                raise ex         
    elif genre:
